 ...
 */
  ...
  class analytics{
    ...
    * @return analytics
    */
    public function __construct($sUser, $sPass, $sClientId){
        $this->_sUser = $sUser;
        $this->_sPass = $sPass;
    ...
    */
    private function auth(){
        ...
        if (isset($_SESSION['ga_auth_'.$this->_sClientId])){
            $this->_sAuth = $_SESSION['ga_auth_'.$this->_sClientId];
            return;
        }
        ...
        $_SESSION['ga_auth_'.$this->_sClientId] = '';
        if (strpos($sResponse, "\n") !== false){
            $aResponse = explode("\n", $sResponse);
            foreach ($aResponse as $sResponse){
                if (substr($sResponse, 0, 4) == 'Auth'){
                    $_SESSION['ga_auth_'.$this->_sClientId] = trim(substr($sResponse, 5));
                }
            }
        }
        if ($_SESSION['ga_auth_'.$this->_sClientId] == ''){
            unset($_SESSION['ga_auth_'.$this->_sClientId]);
            //throw new Exception('Retrieving Auth hash failed!');
            //$_SESSION['ga_error'] = 'login_error';
        }else{
            $this->_sAuth = $_SESSION['ga_auth_'.$this->_sClientId];   
        }
    }
    ...
    private function getUrl($sUrl, $aPost = array(), $aHeader = array()){
        ...
        if (function_exists('curl_init')){
            ...
            if ($sOutput === false){
                ...
                $_SESSION['ga_error'] = 'Wystąpił nieoczekiwany błąd: '.curl_error($rRequest);
            }
            ...
            if ($aInfo['http_code'] != 200){
                ...
                if ($aInfo['http_code'] == 400){
                    ...
                    $_SESSION['ga_error'] = 'Błędne zapytanie: '.$aInfo['http_code'].' w '.$sUrl;
                }
                else if ($aInfo['http_code'] == 403){
                    //throw new Exception('Access denied (' . $aInfo['http_code'] . ') url: ' . $sUrl);
                    $_SESSION['ga_error'] = 'Brak dostępu: '.$aInfo['http_code'].' w '.$sUrl;
                }
                else{
                    //throw new Exception('Not a valid response (' . $aInfo['http_code'] . ') url: ' . $sUrl);
                    $_SESSION['ga_error'] = 'Nieprawidłowa odpowiedź: '.$aInfo['http_code'].' w '.$sUrl;
                }
            }
