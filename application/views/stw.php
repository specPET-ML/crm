...
abstract class AppSTW {
    const ACCESS_KEY = "21ef2061a7c47b8";
    const SECRET_KEY = "fcddb";
    const THUMBNAIL_URI = "inc/thumbnails";
    const THUMBNAIL_DIR = "";
    const CACHE_DAYS = 30; // used 7 for Alexa!
...
*/
    ...
    public static function getXLargeThumbnail($url) {
        return self::getThumbnail($url, array("stwsize"=>"xlg"));
    }
    ...
    public static function getLargeThumbnail($url, $scaler = true, $force = false) {
        if ($scaler)
            return self::getScaledThumbnail($url, 200, 150, array("stwsize"=>"lg"), $force);
    ...
    public static function getSmallThumbnail($url, $scaler = true, $force = false) {
        if ($scaler)
            return self::getScaledThumbnail($url, 120, 90, array("stwsize"=>"sm"), $force);

        return self::getThumbnail($url, array("stwsize"=>"sm"));
    }
}
